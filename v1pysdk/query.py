...
class V1Query(object):
  ...
  def queryAll(self):
    """Forces running of the query so the caller has the option to control when the bulk read query occurs 
       rather than only getting piecemeal queries as various fields are needed."""
    if not self._query_has_run:
      self.run_query()
